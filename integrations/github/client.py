...
class GitHubIntegration(BaseIntegration):
    ...
    def __init__(self, config: GitHubConfig):
        ...
        config.headers = {"Accept": "application/vnd.github.v3+json", "X-GitHub-Api-Version": "2022-11-28"}
            ...
            for card in board.get_all_cards():
                ...
                if card.github_issue_number:
                    ...
                    await self.update_issue(
                        card.github_issue_number, title=card.title, body=card.description, labels=card.labels
                    )
                else:
                    # Create new issue
                    issue = await self.create_issue(title=card.title, body=card.description, labels=card.labels)
    ...
    # Issue Operations
    async def list_issues(self, state: str = "open", labels: List[str] = None) -> List[Dict]:
        """List repository issues."""
        params = {"state": state}
        if labels:
            params["labels"] = ",".join(labels)

        result = await self.request("GET", f"{self._get_repo_base()}/issues", params=params)
        return result

    async def create_issue(
        self, title: str, body: str = "", labels: List[str] = None, assignees: List[str] = None, milestone: int = None
    ) -> Dict:
        """Create a new issue."""
        ...
        result = await self.request("POST", f"{self._get_repo_base()}/issues", data=data)
        return result

    async def update_issue(self, issue_number: int, **kwargs) -> Dict:
        """Update an issue."""
        result = await self.request("PATCH", f"{self._get_repo_base()}/issues/{issue_number}", data=kwargs)
        return result
    ...
    async def list_prs(self, state: str = "open") -> List[Dict]:
        """List pull requests."""
        result = await self.request("GET", f"{self._get_repo_base()}/pulls", params={"state": state})
        return result

    async def get_pr(self, pr_number: int) -> Dict:
        """Get pull request details."""
        result = await self.request("GET", f"{self._get_repo_base()}/pulls/{pr_number}")
        return result

    async def create_pr(self, title: str, head: str, base: str, body: str = "") -> Dict:
        """Create pull request."""
        result = await self.request(
            "POST", f"{self._get_repo_base()}/pulls", data={"title": title, "head": head, "base": base, "body": body}
        )
        return result
    ...
    async def list_workflows(self) -> List[Dict]:
        ...
        result = await self.request("GET", f"{self._get_repo_base()}/actions/workflows")
        return result.get("workflows", [])

    async def trigger_workflow(self, workflow_id: str, ref: str, inputs: Dict = None) -> bool:
    ...
    async def list_workflow_runs(self, workflow_id: str = None, status: str = None) -> List[Dict]:
        """List workflow runs."""
        params = {}
        if status:
...
class GitHubWebhookHandler:
    ...
    async def _handle_issue_event(self, payload: Dict) -> Dict:
        ...
        return {"processed": True, "action": action, "issue_number": issue.get("number"), "title": issue.get("title")}

    async def _handle_pr_event(self, payload: Dict) -> Dict:
        action = payload.get("action")
        pr = payload.get("pull_request", {})

        return {"processed": True, "action": action, "pr_number": pr.get("number"), "merged": pr.get("merged", False)}

    async def _handle_push_event(self, payload: Dict) -> Dict:
        return {"processed": True, "ref": payload.get("ref"), "commits": len(payload.get("commits", []))}
