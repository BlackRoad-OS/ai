...
class SalesforceIntegration(BaseIntegration):
    ...
    async def create_object(self, sobject: str, data: Dict) -> Dict:
        ...
        result = await self.request(
            "POST",
            f"{self._get_api_base()}/sobjects/{sobject}",
            data=data
        )
        return result
    ...
    async def get_object(self, sobject: str, record_id: str) -> Dict:
        ...
        result = await self.request(
            "GET",
            f"{self._get_api_base()}/sobjects/{sobject}/{record_id}"
        )
        return result
    ...
    async def upsert_object(
        ...
        result = await self.request(
            "PATCH",
            f"{self._get_api_base()}/sobjects/{sobject}/{external_field}/{external_id}",
            data=data
        )
        return result
    ...
    async def bulk_create(self, sobject: str, records: List[Dict]) -> Dict:
        ...
        result = await self.request(
            "POST",
            f"{self._get_api_base()}/composite/sobjects",
...
class SalesforceWebhookHandler:
    ...
    async def _handle_card_update(self, payload: Dict) -> Dict:
        ...
        return {
            'processed': True,
            'card_id': payload.get('Card_Id__c'),
            'action': payload.get('Action__c')
        }
    ...
    async def _handle_board_sync(self, payload: Dict) -> Dict:
        ...
        return {
            'processed': True,
            'board_id': payload.get('Board_Id__c'),
            'sync_type': payload.get('Sync_Type__c')
        }
